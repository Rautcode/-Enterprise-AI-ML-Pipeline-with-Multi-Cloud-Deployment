...
terraform {
  ...
  backend "s3" {
    bucket = "aiml-pipeline-terraform-state"
...
resource "aws_ecr_repository" "ml_api" {
  ...
  tags = {
    Name        = "${var.project_name}-${var.environment}-ml-api-ecr"
    Environment = var.environment
    Project     = var.project_name
  }
}
...
resource "aws_ecr_repository" "ml_training" {
  name                 = "${var.project_name}/${var.environment}/ml-training"
  image_tag_mutability = "MUTABLE"
  ...
  tags = {
    Name        = "${var.project_name}-${var.environment}-ml-training-ecr"
    Environment = var.environment
    Project     = var.project_name
  }
}
...
resource "aws_s3_bucket" "ml_artifacts" {
  bucket = "${var.project_name}-${var.environment}-ml-artifacts-${random_string.bucket_suffix.result}"
